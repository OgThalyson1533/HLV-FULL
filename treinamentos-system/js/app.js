...
import { supabase, getSessionUser, onAuthChange, logout, isConfigured, reinitClient } from './supabase.js';
...
async function init() {
  ...
  // Se as credenciais do Supabase ainda não foram configuradas, mostrar tela de setup
  if (!isConfigured()) {
    renderSetup();
    return;
  }
...
// ============================================================
// RENDER: TELA DE CONFIGURAÇÃO (primeiro acesso)
// ============================================================
...
function renderSetup() {
    ...
    <div class="login-container">
      <div class="login-card" style="max-width:440px">
        <div class="login-logo">
          ...
          <p style="color:var(--text-secondary);font-size:12px;margin-top:8px">Configuração inicial do sistema</p>
        </div>
        ...
        ">
          ...
          Acesse <strong>supabase.com → seu projeto → Project Settings → API</strong> e cole as credenciais abaixo.
        </div>

        <div class="form-group">
          <label>Project URL</label>
          <input type="url" id="setup-url" placeholder="https://xxxxxxxxxxx.supabase.co" autocomplete="off" spellcheck="false" />
        </div>

        <div class="form-group">
          <label>anon / public key</label>
          <input type="text" id="setup-key" placeholder="eyJhbGci..." autocomplete="off" spellcheck="false" style="font-family:var(--font-mono);font-size:10px" />
        </div>
        ...
        <button id="btn-setup" class="btn btn-primary btn-full">
          ...
          <span class="btn-loader" style="display:none">
            <span class="spinner"></span> Verificando conexão...
          </span>
        </button>

        <p style="text-align:center;font-size:10px;color:var(--text-tertiary);margin-top:4px">
  ...
  btnSetup.addEventListener('click', async () => {
    ...
    if (!url || !key) {
      errEl.textContent = 'Preencha a URL e a chave anon.';
      errEl.style.display = 'flex';
      return;
    }
    if (!url.startsWith('https://') || !url.includes('.supabase.co')) {
      errEl.textContent = 'URL inválida. Deve ser https://xxxxxx.supabase.co';
      errEl.style.display = 'flex';
      return;
    }

    // Mostrar loader
    btnSetup.querySelector('.btn-text').style.display = 'none';
    btnSetup.querySelector('.btn-loader').style.display = 'inline-flex';
    btnSetup.disabled = true;

    // Testar conexão real antes de salvar
    try {
      const { createClient } = await import('https://esm.sh/@supabase/supabase-js@2');
      const testClient = createClient(url, key);
      const { error } = await testClient.from('perfis').select('id').limit(1);

      if (error && error.message.includes('relation') === false && error.code !== 'PGRST116') {
        throw new Error(error.message);
      }

      // Salvar e reinicializar
      reinitClient(url, key);
      mostrarToast('Supabase conectado com sucesso!', 'success');
      setTimeout(() => init(), 800);

    } catch (err) {
      errEl.textContent = 'Falha na conexão: ' + (err.message || 'verifique a URL e a chave');
      errEl.style.display = 'flex';
      btnSetup.querySelector('.btn-text').style.display = 'inline';
      btnSetup.querySelector('.btn-loader').style.display = 'none';
...
function renderApp() {
    ...
    <div class="app-shell">
      ...
      <aside class="sidebar" id="sidebar">
        ...
        <div class="sidebar-footer">
          ...
          <button class="btn-icon" id="btn-reconfig" title="Reconfigurar Supabase" style="color:var(--text-tertiary)">
  ...
  if (btnReconfig) {
    btnReconfig.addEventListener('click', () => {
      if (confirm('Isso irá desconectar o Supabase e abrir a tela de configuração. Continuar?')) {
        localStorage.removeItem('hlv_supabase_url');
        localStorage.removeItem('hlv_supabase_key');
        location.reload();
      }
    });
  }
